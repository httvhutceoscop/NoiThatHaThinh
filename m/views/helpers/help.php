...
class HelpHelper extends AppHelper {
	...
	function getShopingCart()
	{
		...
		function getComment($id=null){
		 ...
		 function bth_get_youtube_id($youtube_url = null)
        {
            if($youtube_url == '' || is_null($youtube_url)) return false;
            
            $arr = parse_url($youtube_url);
            $buff = explode('&', $arr['query']);
            $yID = '';
            ...
            return array('id' => $yID, 'img' => 'http://img.youtube.com/vi/'. $yID . '/1.jpg');
        }
