...
<?php echo $form->create(null, array( 'url' => DOMAINAD.'origins/search','type' => 'post','enctype'=>'multipart/form-data','name'=>'image')); ?> 
                ...
                <tbody>
                   <?php $i=1; foreach ($catproduct as $key =>$value){?>
                    <tr>
                        ...
                        <td><?php echo $value['Origin']['char'];?></td>
                        <td><?php echo date('d-m-Y', strtotime($value['Origin']['created'])); ?></td>
                        <?php if($value['Origin']['status']==0){?>  
                          <td>
                             <a href="<?php echo DOMAINAD?>catproducts/edit/<?php echo $value['Origin']['id'] ?>" title="Edit"><img src="<?php echo DOMAINAD?>images/icons/pencil.png" alt="Edit" /></a>
                             <a href="javascript:confirmDelete('<?php echo DOMAINAD?>catproducts/delete/<?php echo $value['Origin']['id'] ?>')" title="Delete"><img src="<?php echo DOMAINAD?>images/icons/cross.png" alt="Delete" /></a> 
                             <a href="<?php echo DOMAINAD?>Catproducts/active/<?php echo $value['Origin']['id'] ?>" title="Kích hoạt" class="icon-5 info-tooltip"><img src="<?php echo DOMAINAD?>images/icons/Play-icon.png" alt="Kích hoạt" /></a>
                        </td>
                        <?php } else {?> 
                          <td>
                            <!-- Icons -->
