...
class ProductsController extends AppController {
	...
	var $name = 'Products';
	var $uses=array('Product','Catproduct','Comment','Manufacturer');
	var $helpers = array('Html','Ajax','Javascript');
	var $components = array( 'RequestHandler' );
	function index() {
		//list danh sach tin tuc
		mysql_query("SET names utf8");
		$this->paginate = array('conditions'=>array('Product.status'=>1),'order'=>'Product.id DESC','limit'=>18);
    ...
    function thietbivp() {
		...
		$this->paginate = array('conditions'=>array('Product.status'=>1,'Product.catproduct_id'=>$dem),'order'=>'Product.id DESC','limit'=>12);
	    $this->set('products', $this->paginate('Product',array()));	
        $this->set('newsproducts', $this->Product->find('all',array('conditions'=>array('Product.status'=>1,'Product.catproduct_id'=>$dem,'Product.sptieubieu'=>'1'),'limit'=>21,'order' => 'Product.id DESC')));
	}
	...
	function listproduct($id=null) {
		...
		mysql_query("SET names utf8");       
      	$this->set('newsproducts', $this->Product->find('all',array('conditions'=>array('Product.catproduct_parent_id'=>$id),'limit'=>30,'order' => 'Product.id ASC')));
		 $this->paginate = array('conditions'=>array('Product.catproduct_parent_id'=>$id),'order'=>'Product.id ASC','limit'=>30);
	      $this->set('products', $this->paginate('Product',array()));	
		  $x=$this->Catproduct->read(null,$id);
		  $this->set('cat',$this->Catproduct->read(null,$id));	
		  //$this->set('title',$x['Catproduct']['name']);
			//$this->set('keywords',$x['Catproduct']['keywords']);
			//$this->set('description',$x['Catproduct']['description']);
	}
	function listproductchild($id=null) {
		//list danh sach tin tuc Catproduct
		mysql_query("SET names utf8");       
      	$this->set('newsproducts', $this->Product->find('all',array('conditions'=>array('Product.catproduct_id'=>$id),'limit'=>30,'order' => 'Product.id ASC')));
		 $this->paginate = array('conditions'=>array('Product.catproduct_id'=>$id),'order'=>'Product.id ASC','limit'=>30);
	      $this->set('products', $this->paginate('Product',array()));	
		  $x=$this->Catproduct->read(null,$id);		  
		  $this->set('catchild',$this->Catproduct->read(null,$id));	
		  $this->set('cat',$this->Catproduct->read(null,$x['Catproduct']['parent_id']));	
		  //$this->set('title',$x['Catproduct']['name']);
			//$this->set('keywords',$x['Catproduct']['keywords']);
			//$this->set('description',$x['Catproduct']['description']);
	}
		function binhnuocnong($id=null) {
		//list danh sach tin tuc Catproduct
		mysql_query("SET names utf8");       
      	$this->set('newsproducts', $this->Product->find('all',array('conditions'=>array('Product.status'=>1,'Product.binhnuocnong'=>1),'limit'=>21,'order' => 'Product.id DESC')));
		 $this->paginate = array('conditions'=>array('Product.status'=>1,'Product.binhnuocnong'=>1),'order'=>'Product.id DESC','limit'=>21);
	      $this->set('products', $this->paginate('Product',array()));	
		  
	}
			function maylocnuoc($id=null) {
		//list danh sach tin tuc Catproduct
		mysql_query("SET names utf8");       
      	$this->set('newsproducts', $this->Product->find('all',array('conditions'=>array('Product.status'=>1,'Product.maylocnuoc'=>1),'limit'=>21,'order' => 'Product.id DESC')));
		 $this->paginate = array('conditions'=>array('Product.status'=>1,'Product.maylocnuoc'=>1),'order'=>'Product.id DESC','limit'=>21);
	      $this->set('products', $this->paginate('Product',array()));	
		  
	}
			function sanphamkhuyenmai($id=null) {
		//list danh sach tin tuc Catproduct
		mysql_query("SET names utf8");       
      	$this->set('newsproducts', $this->Product->find('all',array('conditions'=>array('Product.status'=>1,'Product.spkhuyenmai'=>1),'limit'=>21,'order' => 'Product.id DESC')));
		 $this->paginate = array('conditions'=>array('Product.status'=>1,'Product.spkhuyenmai'=>1),'order'=>'Product.id DESC','limit'=>21);
	      $this->set('products', $this->paginate('Product',array()));	
	...
	function listproducthsx($id=null) {
		//list danh sach tin tuc Catproduct
		mysql_query("SET names utf8");       
      	$this->set('newsproducts', $this->Product->find('all',array('conditions'=>array('Product.status'=>1,'Product.manufacturer'=>$id),'limit'=>21,'order' => 'Product.id DESC')));
		 $this->paginate = array('conditions'=>array('Product.status'=>1,'Product.manufacturer'=>$id),'order'=>'Product.id DESC','limit'=>21);
	      $this->set('products', $this->paginate('Product',array()));	
		  $this->set('cat',$this->Catproduct->read(null,$id));	
		  $this->set('cat1',$this->Manufacturer->read(null,$id));
		  $x=$this->Manufacturer->read(null,$id);	
		  $this->set('title',$x['Manufacturer']['name']);
		$this->set('keywords',$x['Manufacturer']['keywords']);
		$this->set('description',$x['Manufacturer']['description']);
	}
...
$this->paginate = array('conditions'=>array('Product.status'=>1,'Product.title LIKE'=> "%$s%",'Product.manufacturer LIKE'=> "%$hang%",'Product.catproduct_id LIKE'=> "%$system%"),'limit' => '8','order' => 'Product.id DESC');
	...
	function view($id=null) {
		...
		$this->set('catchild', $this->Catproduct->read(null, $name['Product']['catproduct_id']));
		$this->set('cat', $this->Catproduct->read(null, $name['Product']['catproduct_parent_id']));
        $this->set('title',$name['Product']['title']);
		$this->set('keywords',$name['Product']['keywords']);
		$this->set('description',$name['Product']['description']);
  		$this->set('views',$name);		
        $this->paginate = array('conditions'=>array('Product.status'=>1,'Product.catproduct_id'=>$name['Product']['catproduct_id'],'Product.id <>'=>$name['Product']['id']),'order'=>'Product.id DESC','limit'=>9);
		...
		if(!isset($_SESSION['daxem'])){  $_SESSION['daxem']=array();};
		 ...
		 {   
			 ...
			 $daxem=$_SESSION['daxem'];
			 if(!isset($daxem[$id]))
			 {	
				$_SESSION['daxem'][$id]=$id;
			 }
		}
